...
public class Node {
	...
	public boolean insert(DataEntry dataEntry) {
		// Ensure that point lies in this quad
		...
		// Check if Quad still has empty slots
	...
	public boolean liesInBoundingBox(GeoLocation bb_upperRight,
			GeoLocation bb_lowerLeft) {
	...
	private void subdivide() {
		...
		double latitude_diff = upperRight.getLatitude()
				- lowerLeft.getLatitude();
		double longitude_diff = upperRight.getLongitude()
				- lowerLeft.getLongitude();

		children[0] = new Node(maxCapacity, new GeoLocation(
				upperRight.getLatitude(), lowerLeft.getLongitude()
						+ longitude_diff / 2), new GeoLocation(
				lowerLeft.getLatitude() + latitude_diff / 2,
				lowerLeft.getLongitude()));

		children[1] = new Node(maxCapacity, new GeoLocation(
				upperRight.getLatitude(), upperRight.getLongitude()),
				new GeoLocation(lowerLeft.getLatitude() + latitude_diff / 2,
						lowerLeft.getLongitude() + longitude_diff / 2));

		children[2] = new Node(maxCapacity, new GeoLocation(
				lowerLeft.getLatitude() + latitude_diff / 2,
				lowerLeft.getLongitude() + longitude_diff / 2),
				new GeoLocation(lowerLeft.getLatitude(), lowerLeft
						.getLongitude()));

		children[3] = new Node(maxCapacity, new GeoLocation(
				lowerLeft.getLatitude() + latitude_diff / 2,
				upperRight.getLongitude()), new GeoLocation(
	...
	public void printIds() {
		System.out.println("Ids in " + "upperRight: "
				+ upperRight.getLatitude() + "," + upperRight.getLongitude()
	...
	public void printInternalNode() {
		System.out.println("Internal Node " + "upperRight: "
				+ upperRight.getLatitude() + "," + upperRight.getLongitude()
				+ " , lowerLeft: " + lowerLeft.getLatitude() + ","
				+ lowerLeft.getLongitude());
	}
	...
	public static void main(String[] args) {
		...
		System.out.print("UpperRight => latitude= "
				+ testNode.upperRight.getLatitude() + " longitude= "
				+ testNode.upperRight.getLongitude());
		System.out.println(",, LowerLeft => latitude= "
				+ testNode.lowerLeft.getLatitude() + " longitude= "
				+ testNode.lowerLeft.getLongitude());
		System.out.println();
		for (Node node : testNode.children) {
			System.out.println(node);
			System.out.print("UpperRight => latitude= "
					+ node.upperRight.getLatitude() + " longitude= "
					+ node.upperRight.getLongitude());
			System.out.println(",, LowerLeft => latitude= "
					+ node.lowerLeft.getLatitude() + " longitude= "
					+ node.lowerLeft.getLongitude());
			System.out.println();
		}
