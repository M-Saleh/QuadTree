...
public class QuadTree {
	...
	public QuadTree(int maxCapacity, GeoLocation upperRight, GeoLocation lowerLeft) {
		rootNode = new QuadTreeNode(maxCapacity, upperRight, lowerLeft);
	}
	...
	private void search(ArrayList<DataEntry> dataEntries, QuadTreeNode node, GeoLocation upperRight,
	...
	private void getAllChildren(ArrayList<DataEntry> dataEntries, QuadTreeNode node) {
		if (node.children != null) {
			for (QuadTreeNode childNode : node.children) {
	...
	public void printAllTree(QuadTreeNode root) {
		...
		for (QuadTreeNode node : root.children) {
			printAllTree(node);
		}
	}
}
